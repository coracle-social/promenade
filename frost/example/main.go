...
import (
	...
	"github.com/nbd-wtf/go-nostr/frost"
)
...
func (pe ParticipantError) Error() string {
	...
	return fmt.Sprintf("participant %d (%s/%d) failed: %s", pe.index, pks.PublicKey.Hex(), pks.ID, pe.reason)
}
...
func flow(
	...
	shards, pubkey, _ := frost.TrustedDealerKeygen([32]byte(secret), threshold, totalSigners)
...
func coordinator(ctx context.Context, cfg *frost.Configuration, signers []chan string, messageHex string) (string, error) {
...
func signer(ch chan string, shard *keys.KeyShare) {
